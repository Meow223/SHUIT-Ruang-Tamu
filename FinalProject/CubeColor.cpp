...
CubeColor::CubeColor(Shader* shader)
{
	this->shader = shader;
	angle = vao = vbo = ebo = 0;
}
...
void CubeColor::SetPosition(float x, float y, float z)
{
	position.x = x;
	position.y = y;
	position.z = z;
...
void CubeColor::SetScale(float sx, float sy, float sz)
{
	scale.x = sx;
	scale.y = sy;
	scale.z = sz;
...
void CubeColor::SetRotation(float angle, float x, float y, float z)
{
	this->angle = angle;
	rotationAxis.x = x;
	rotationAxis.y = y;
