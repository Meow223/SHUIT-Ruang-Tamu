...
Glass::Glass(Shader* shader, Texture* diffuseTexture, Texture* specularTexture)
{
	this->shader = shader;
...
void Glass::SetPosition(float x, float y, float z)
{
	position.x = x;
	position.y = y;
	position.z = z;
...
void Glass::SetScale(float sx, float sy, float sz)
{
	scale.x = sx;
	scale.y = sy;
	scale.z = sz;
...
void Glass::SetRotation(float angle, float x, float y, float z)
{
	this->angle = angle;
	rotationAxis.x = x;
	rotationAxis.y = y;
